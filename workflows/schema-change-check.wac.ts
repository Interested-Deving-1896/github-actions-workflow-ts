...
const checkExistingPR = new Step({
  ...
  with: {
    script: dedentString(`
      ...
      // The \`head\` filter of pulls.list only matches an exact branch name, so list all open PRs and filter by prefix.
      ...
      const schemaUpdatePRs = pulls.filter(pr => pr.head.ref.startsWith('chore/schema-update-'));
